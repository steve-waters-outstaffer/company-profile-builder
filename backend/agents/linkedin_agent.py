...
class LinkedInAgent:
    ...
    def get_company_data(self, company_name: str, provided_url: Optional[str] = None) -> Dict:
        ...
        if linkedin_url:
            ...
            logging.warning("LinkedIn scrape failed or returned invalid data. Falling back to website.")
    ...
    def _find_website_url(self, company_name: str) -> Optional[str]:
        # Helper if we only have a name and LinkedIn failed
        try:
    ...
    def _scrape_website_fallback(self, company_name: str, url: str) -> Dict:
        ...
        try:
            ...
            if response.ok:
                ...
                # Quick LLM summary to mimic a LinkedIn-like profile shape (simple + resilient)
                prompt = (
            ...
            logging.error(f"Website scrape failed with status {response.status_code}: {response.text}")
        except Exception as e:
